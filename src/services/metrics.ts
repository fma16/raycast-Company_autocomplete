...
class MetricsCollector {
   ...
   */
  getStats(timeRangeMs: number = 3600000): PerformanceStats { // Default: 1 hour
    const now = Date.now();
  ...
  private logPerformanceWarnings(metric: ApiMetrics): void {
    ...
    if (metric.responseTime > 5000) { // >5s
...
export class PerformanceMonitor {
   ...
   */
  static isHealthy(timeRangeMs: number = 1800000): boolean { // Default: 30 minutes
